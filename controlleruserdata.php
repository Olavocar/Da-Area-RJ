...
if($data_check){
    ...
    // Se o usuário clicar no botão de continuar no formulário de esqueci a senha
    if(isset($_POST['check-email'])){
        ...
        if(mysqli_num_rows($run_sql) > 0){
            ...
            if($run_query){
                ...
                $sender = "From: daarearj@gmail.com";
                if(mail($email, $subject, $message, $sender)){
                    $info = "Enviamos um código de redefinição de senha para o seu e-mail - $email";
                    $_SESSION['info'] = $info;
                    $_SESSION['email'] = $email;
            ...
            $errors['email'] = "Este e-mail não existe!";
        }
    }
    
    // Se o usuário clicar no botão de verificar código de redefinição
    ...
    // Se o usuário clicar no botão de mudar senha
    if(isset($_POST['change-password'])){
        ...
        if($senha !== $csenha){
            ...
            // obtendo este e-mail usando a sessão
            $encpass = password_hash($senha, senha_BCRYPT);
            $update_pass = "UPDATE cadastro SET code = $code, senha = '$encpass' WHERE email = '$email'";
            $run_query = mysqli_query($con, $update_pass);
            if($run_query){
                $info = "Você alterou sua senha com sucesso. Agora você pode fazer login com sua nova senha.";
                $_SESSION['info'] = $info;
    ...
    // Se o botão de login agora for clicado
